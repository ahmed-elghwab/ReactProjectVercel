    ...
    <Helmet>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
        {filteredProducts.map((product) => (
          <div key={product.id || product._id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all ease-out duration-800 hover:-translate-y-1 relative group flex flex-col">
            {/* Like Button */}
            <button 
            ...
            >
              <FontAwesomeIcon 
                icon={likedProducts.has(product._id) ? solidHeart : regularHeart}
                className={`text-xl ${likedProducts.has(product._id) ? 'text-red-500' : 'text-gray-400'}`}
              />
            </button>
